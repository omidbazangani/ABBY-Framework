 ...
 * Yao, Yuan, Mo Yang, Conor Patrick, Bilgiday Yuce, and Patrick Schaumont.
 * "Fault-assisted side-channel analysis of masked implementations." In 2018
...
void init_masked_round_keys() {
  ...
  int i;
  for (i = 0; i < 10; i++) {
    remask(RoundKey_masked[i], Mask[6], Mask[7], Mask[8], Mask[9],
           Mask[4] ^ WordMask[0], Mask[4] ^ WordMask[1], Mask[4] ^ WordMask[2],
           Mask[4] ^ WordMask[3]);
  }

  // 1st RK Mask m' to 0
  remask(RoundKey_masked[10], 0, 0, 0, 0, Mask[5] ^ WordMask[0],
         Mask[5] ^ WordMask[1], Mask[5] ^ WordMask[2], Mask[5] ^ WordMask[3]);
}
void copy_key() {
...
void aes128(uint8_t *state) {
  ...
  for (i = 1; i < 10; i++) {
    ...
    remask(state, Mask[0], Mask[1], Mask[2], Mask[3], Mask[5] ^ WordMask[0],
           Mask[5] ^ WordMask[1], Mask[5] ^ WordMask[2], Mask[5] ^ WordMask[3]);
    mixColumns(state);
