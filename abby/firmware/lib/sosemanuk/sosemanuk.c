...
#ifdef SOSEMANUK_ECRYPT
void ECRYPT_keysetup(ECRYPT_ctx *kc, const u8 *key, u32 keysize, u32 ivsize)
#else
/* see sosemanuk.h */
void sosemanuk_schedule(sosemanuk_key_context *kc, unsigned char *key,
                        size_t key_len)
#endif
{
  /*
...
#define WUP1(cc)                                                               \
  ...
  printf("  -> %08lX %08lX %08lX %08lX %08lX %08lX %08lX %08lX\n",
         (unsigned long)w7, (unsigned long)w6, (unsigned long)w5,
         (unsigned long)w4, (unsigned long)w3, (unsigned long)w2,
...
#ifdef SOSEMANUK_VECTOR
  {
    ...
    for (u = 0; u < 100; u += 4) {
      printf("Serpent24 subkey %2u:"
             " %08lX %08lX %08lX %08lX\n",
             u / 4, (unsigned long)kc->sk[u + 3], (unsigned long)kc->sk[u + 2],
             (unsigned long)kc->sk[u + 1], (unsigned long)kc->sk[u + 0]);
    }
  }
#endif
...
#else
/* see sosemanuk.h */
void sosemanuk_init(sosemanuk_run_context *rc, sosemanuk_key_context *kc,
...
static void sosemanuk_internal(ECRYPT_ctx *rc, const u32 *src, u32 *dst)
#elif defined SOSEMANUK_SPEED
static unum32 sosemanuk_internal(sosemanuk_run_context *rc,
...
#define LRU(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, dd)                        \
  do {                                                                         \
    ...
    PLFSR(dd, s##x1, s##x2, s##x3, s##x4, s##x5, s##x6, s##x7, s##x8, s##x9,   \
          s##x0);                                                              \
  } while (0)
...
#define PSPIN(x0, x1, x2, x3)                                                  \
  do {                                                                         \
    printf("Serpent1 input:  %08lX %08lX %08lX %08lX\n", (unsigned long)x3,    \
           (unsigned long)x2, (unsigned long)x1, (unsigned long)x0);           \
  } while (0)
...
/* see sosemanuk.h */
void sosemanuk_prng(sosemanuk_run_context *rc, unsigned char *out,
                    size_t out_len) {
...
/* see sosemanuk.h */
void sosemanuk_encrypt(sosemanuk_run_context *rc, unsigned char *in,
                       unsigned char *out, size_t data_len) {
  if (rc->ptr < (sizeof rc->buf)) {
    size_t rlen = (sizeof rc->buf) - rc->ptr;
 ...
 */
static void maketest(int tvn, unsigned char *key, size_t key_len,
                     unsigned char *iv, size_t iv_len) {
#ifdef SOSEMANUK_ECRYPT
  ECRYPT_ctx ctx;
#else
...
int main(void) {
  ...
  static unsigned char iv1[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                                0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};

  static unsigned char key2[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
                                 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
                                 0xCC, 0xDD, 0xEE, 0xFF};
  static unsigned char iv2[] = {0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
...
int main(int argc, char *argv[]) {
  ...
  static unsigned char iv[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                               0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
  sosemanuk_key_context kc;
  sosemanuk_run_context rc;
